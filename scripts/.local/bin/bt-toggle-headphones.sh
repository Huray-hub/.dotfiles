...
main() {
	...
	is_bt_device_connected "$deviceInfo" &&
		action=disconnect ||
		action=connect
	...
	bluetoothctl "$action" "$deviceUid" &&
		notify-send "$deviceName ${action}ed successfully" ||
		notify-send "Could not $action $deviceName " -u critical
